...
import { X, CheckCircle2, ShieldCheck, Zap, Code2, Sliders } from 'lucide-react';
import type { Project } from '../data/portfolioData';
import { ChargePilotSimulator, QCAnalyticsSimulator, KoraVisionSimulator } from './InteractiveSimulators';
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Tab Switcher: Architecture Breakdown vs Live Feature Simulator */}
        <div className="px-4 sm:px-6 pt-3 sm:pt-4 pb-3 border-b border-white/10 flex flex-wrap sm:flex-nowrap items-center justify-between gap-2 bg-white/[0.01]">
          <div className="flex gap-2 w-full sm:w-auto">
            <button
              onClick={() => setActiveTab('architecture')}
              className={`flex-1 sm:flex-none px-3 sm:px-4 py-2 rounded-xl text-[11px] sm:text-xs font-bold transition-all flex items-center justify-center gap-1.5 ${
                activeTab === 'architecture'
                  ? 'apple-glass-button-primary text-white'
                  : 'apple-glass-button text-slate-400 hover:text-white'
              }`}
            >
          ...
          <span className="text-[11px] font-mono text-slate-400 hidden sm:inline">Role: {project.role}</span>
        </div>
          ...
          {activeTab === 'architecture' ? (
            ...
            <div className="space-y-4">
              <p className="text-xs text-slate-300">
                Interactive real-time widget simulating the core frontend logic built for <strong>{project.title}</strong>:
              </p>
              {project.id === 'charge-pilot-pro' && <ChargePilotSimulator />}
              {project.id === 'qc-analytics-portal' && <QCAnalyticsSimulator />}
              {project.id === 'koravision' && <KoraVisionSimulator />}
            </div>
          )}
